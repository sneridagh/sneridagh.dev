...
2. Go to your repository at [sneridagh/sneridagh-dev](https://github.com/sneridagh/sneridagh-dev).
3. Click on `Settings` at the top-right corner.
4. In the left sidebar, click on `Environments`.
5. Press `New environment`.
6. Name the environment `sneridagh.dev` and hit `Configure environment`.
...
|-------------|--------------|-------------|
| DEPLOY_HOST | sneridagh.dev | The hostname or IP address of your Docker Swarm manager. |
...
- [Backend Tests Workflow](https://github.com/sneridagh/sneridagh-dev/actions/workflows/backend.yml)
- [Frontend Tests Workflow](https://github.com/sneridagh/sneridagh-dev/actions/workflows/frontend.yml)

Upon successful completion of the tests, Docker images for the Backend (`ghcr.io/sneridagh/sneridagh-dev-backend`) and Frontend (`ghcr.io/sneridagh/sneridagh-dev-frontend`) will be available.

### Initiating Manual Deployment

1. Navigate to [Manual Deployment of sneridagh.dev](https://github.com/sneridagh/sneridagh-dev/actions/workflows/manual-deploy.yml).
...
- Initiate a new deployment using the `devops/stacks/sneridagh.dev.yml` stack.
